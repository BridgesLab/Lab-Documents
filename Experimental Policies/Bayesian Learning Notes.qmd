...
Lets say we have three categories with observed counts `r counts`.  We set a prior of $Dirichlet(\frac{1}{2},\frac{1}{2},\frac{1}{2})$. Using this, our updated posterior is $Dirichlet(\frac{1}{2}+`r counts[1]`,\frac{1}{2}+`r counts[2]`,\frac{1}{2}+`r counts[3]`)=Dirichlet(`r 0.5+counts`)$.  Based on this, the posterior mean is `r (0.5+counts)/(sum(0.5+counts))` with a variance of `r ((0.5+counts)/sum(0.5+counts))*(1-(0.5+counts)/sum(0.5+counts))/(sum(0.5+counts)+1)`.  This is visualized here:
